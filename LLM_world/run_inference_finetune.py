...
Single-shard run (one GPU):
  python LLM_world/run_inference_finetune.py --shard 0 --num-shards 7

Launch all shards in parallel:
  bash LLM_world/launch_inference_finetune.sh
'''
import argparse
import sys
import os
...
RESULTS_DIR  = "LLM_world/results_finetune"
...
def compute_ylims(model, indices):
    """Compute per-feature (ymin, ymax) across all samples in indices."""
    n_feat = 11
    lo = [float("inf")]  * n_feat
    hi = [float("-inf")] * n_feat
