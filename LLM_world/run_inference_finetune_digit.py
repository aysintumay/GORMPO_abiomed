'''
Inference for the digit-token finetuned model.

Single-shard:
  python LLM_world/run_inference_finetune_digit.py --shard 0 --num-shards 7

Launch all shards in parallel:
  bash LLM_world/launch_inference_finetune_digit.sh
'''
import argparse
import sys
import os
...
ADAPTER_PATH = "LLM_world/checkpoints_finetune_digit/final"
VIZ_SAMPLES  = [0, 1, 2, 3]
VIZ_OUT_DIR  = "LLM_world/figures_finetune_digit"
RESULTS_DIR  = "LLM_world/results_finetune_digit"
...
def _sample_arrays(model, idx):
    """Return (ctx_phys, y_phys, pred_phys, std_phys, pl_int) for one sample."""
...
def compute_ylims(model, indices):
    n_feat = 11
    lo = [float("inf")]  * n_feat
    hi = [float("-inf")] * n_feat

    for idx in indices:
        ctx_phys, y_phys, pred_phys, std_phys, _ = _sample_arrays(model, idx)
        mu  = pred_phys.numpy()
        sig = std_phys.numpy()
        ...
        for j in range(n_feat):
            ...
            lo[j] = min(lo[j], float(all_vals.min()))
            hi[j] = max(hi[j], float(all_vals.max()))
...
def plot_sample(model, idx, out_dir, ylims=None):
    ctx_phys, y_phys, pred_phys, std_phys, pl_int = _sample_arrays(model, idx)
    ...
    axes = axes.flatten()
    fig.suptitle(f"Test sample {idx}  |  P-level = {pl_int}  |  finetuned (digit)", fontsize=12)
...
model = FinetunedDigitTokenWorldModel(model_id=MODEL_ID, adapter_path=ADAPTER_PATH, device=DEVICE)
model.load_data(DATA_PATH)

n_test  = len(model.data_test)
indices = list(range(args.shard, n_test, args.num_shards))
print(f"[shard {args.shard}/{args.num_shards}] device={DEVICE} "
...
if args.shard == 0:
    ...
    ylims = compute_ylims(model, VIZ_SAMPLES)
    for idx in VIZ_SAMPLES:
        plot_sample(model, idx, VIZ_OUT_DIR, ylims=ylims)
