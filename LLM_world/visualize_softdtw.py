"""
Inference figures for the SoftDTW-trained Transformer world model.
Mirrors visualize_transformer.py — dropout sampling for uncertainty.

Run from the project root:
    python LLM_world/visualize_softdtw.py

Saves to LLM_world/figures_softdtw/
"""
import sys
import os
import torch
...
NUM_SAMPLES = 10
SAMPLE_IDXS = [0, 1, 2, 3]
OUT_DIR     = os.path.join(os.path.dirname(__file__), "figures_softdtw")
...
# ── pre-pass: compute shared y-limits across all samples ─────────────────────
print("Computing shared y-limits ...", flush=True)
lo = [float("inf")]  * n_feat
hi = [float("-inf")] * n_feat
...
for idx in SAMPLE_IDXS:
    ...
    with torch.no_grad():
        samples = [
            model.model(x, pl).reshape(model.forecast_horizon, n_feat)
            for _ in range(NUM_SAMPLES)
        ]
    model.model.eval()

    samples   = torch.stack(samples)
    mean_norm = samples.mean(0)
    std_norm  = samples.std(0)

    std_t  = model.std[cols_no_pl].cpu()
    mean_t = model.mean[cols_no_pl].cpu()
    pred_phys = mean_norm.cpu() * std_t + mean_t
    std_phys  = std_norm.cpu()  * std_t
    ctx_phys  = x[0].cpu() * model.std[cols].cpu() + model.mean[cols].cpu()
    y_phys    = (torch.as_tensor(y).float().reshape(model.forecast_horizon, n_feat)
                 * model.std[cols_no_pl].cpu() + model.mean[cols_no_pl].cpu())
...
for idx in SAMPLE_IDXS:
    ...
    axes = axes.flatten()
    fig.suptitle(f"Test sample {idx}  |  P-level = {pl_int}  |  Transformer (SoftDTW)", fontsize=12)
