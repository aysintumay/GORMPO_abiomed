...
Single-shard run (one GPU):
  python LLM_world/run_inference_digit.py --shard 0 --num-shards 7 --k-shot 5

Launch all shards:
  bash LLM_world/launch_inference_digit.sh
'''
import argparse
import sys
import os
...
from LLM_world.world_model import FewShotDigitTokenWorldModel, FEATURE_NAMES
...
DEVICE = args.device if args.device else f"cuda:{args.shard}"
VIZ_OUT_DIR = f"LLM_world/figures_digit_k{args.k_shot}"
RESULTS_DIR = f"LLM_world/results_digit_k{args.k_shot}"
...
def plot_sample(model, idx, out_dir):
    ...
    axes = axes.flatten()
    fig.suptitle(f"Test sample {idx}  |  P-level = {pl_int}  |  digit-token {args.k_shot}-shot", fontsize=12)
...
model = FewShotDigitTokenWorldModel(model_id=MODEL_ID, device=DEVICE, k_shot=args.k_shot)
