...
Single-shard run (one GPU):
  python LLM_world/run_inference.py --shard 0 --num-shards 8

Launch all 8 shards in parallel (see launch_inference.sh):
  bash LLM_world/launch_inference.sh
'''
import argparse
import sys
import os
...
VIZ_SAMPLES = [0, 1, 2, 3]   # only generated by shard 0
VIZ_OUT_DIR = "LLM_world/figures"
...
def predict_autoregressive(model, context_norm, p_levels):
    ctx = context_norm.float()
    means, stds = [], []
    for pl in p_levels:
        mean_h, std_h = model.predict(ctx, p_level_int=pl)
        means.append(mean_h)
        stds.append(std_h)
        ctx = mean_h
...
def plot_sample(model, idx, out_dir):
    ...
    axes = axes.flatten()
    fig.suptitle(f"Test sample {idx}  |  P-level = {pl_int}", fontsize=12)
...
if args.shard == 0:
    x0, pl0, _ = model.data_test[0]
    context0   = torch.as_tensor(x0)
    pl0_int    = int(round(model.unnorm_pl(torch.as_tensor(pl0).float().mean()).item()))

    mean_norm0, _ = model.predict(context0, p_level_int=pl0_int)
    mean_phys0    = model.unnorm_output(mean_norm0)

    header = f"{'t_min':>6}  " + "  ".join(f"{n:>12}" for n in FEATURE_NAMES)
    print("\n=== 1-hour direct predictions — sample 0 (physical units) ===")
    print(header)
    for i in range(model.forecast_horizon):
        t    = (i + 1) * 10
        vals = "  ".join(f"{mean_phys0[i, j].item():12.2f}" for j in range(model.num_features))
        print(f"{t:6d}  {vals}")
    ...
    print("\nRunning autoregressive 2-hour prediction (sample 0)...")
    auto_mean, _ = predict_autoregressive(model, context0, p_levels=[pl0_int, pl0_int])
    auto_phys    = model.unnorm_output(auto_mean)
    print("\n=== 2-hour autoregressive predictions — sample 0 (physical units) ===")
    print(header)
    for i in range(auto_phys.shape[0]):
        t    = (i + 1) * 10
        src  = "(direct)" if i < model.forecast_horizon else "(autoreg)"
        vals = "  ".join(f"{auto_phys[i, j].item():12.2f}" for j in range(model.num_features))
        print(f"{t:6d}  {vals}  {src}")
