...
```bash
# reuse existing:
export DATASET_PATH=$(pwd)/../GORMPO_abiomed/synthetic_data/SAC_5000eps_stochastic.npz

# OR regenerate (slower, from the trained SAC policy already in the repo):
cd ../GORMPO_abiomed/abiomed_env
python sample_offline_dataset.py --model_path data/sac_20250827_0837.zip \
...
```bash
cd LEQ
export DATASET_PATH=<path from above>
bash bash_scr/leq_mcs.sh
```
...
```bash
cd LEQ
bash bash_scr/leq_d4rl_sparse.sh
```
